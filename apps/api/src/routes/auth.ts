...
import { verifyJWT, getAccessTokenExpiry, getRefreshTokenExpiry, type RefreshTokenPayload } from "../lib/jwt";
...
authRoutes.post("/google/exchange", async (c) => {
  ...
  try {
    ...
    const { accountId, isNew: isNewAccount } = await connectGmailAccount(user.id, userInfo.email!, tokens);
...
authRoutes.post("/refresh", async (c) => {
  ...
  if (!refreshToken || !fingerprint) {
    return c.json({ error: "invalid_refresh_token", message: "Session expired. Please log in again." }, 401);
  }
...
authRoutes.delete("/account", auth(), async (c) => {
  ...
  if (body.confirmation !== "DELETE") {
    return c.json({ error: "confirmation_required", message: "Send { confirmation: \"DELETE\" } to confirm" }, 400);
  }
