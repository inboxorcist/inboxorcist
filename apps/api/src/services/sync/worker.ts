...
export async function processMetadataSync(data: SyncJobData): Promise<void> {
  ...
  if (await hasRunningSync(accountId)) {
    console.log(`[SyncWorker] Another sync already running for account ${accountId}, skipping job ${jobId}`);
  ...
  try {
    ...
    console.log(
      `[SyncWorker] Sync job ${jobId} completed. Processed ${processedCount} emails.`
    );
  } catch (error) {
...
export async function processDeltaSync(
  ...
  try {
    ...
    if (changes.messagesAdded.length > 0) {
      ...
      const emails = await fetchMessageDetails(accountId, messageIds, throttle, (processed, failed) => {
        if (processed % 100 === 0) {
...
export async function startDeltaSync(
  accountId: string
): Promise<{ type: "delta"; result: { added: number; deleted: number } } | { type: "full"; job: Job }> {
  const account = await getAccount(accountId);
  if (!account) {
 ...
 */
export async function startMetadataSync(
  accountId: string,
  ...
  if (activeJob) {
    console.log(`[SyncWorker] Active sync job ${activeJob.id} already exists for account ${accountId}`);
    return activeJob;
  }
