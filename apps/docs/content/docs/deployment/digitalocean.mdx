...
Mark `JWT_SECRET` and `ENCRYPTION_KEY` as **Secret**.
...
If no `DATABASE_URL` is set, Inboxorcist uses SQLite automatically.

<Callout type="warn">
  SQLite data may not persist across DigitalOcean deployments. For production, use managed PostgreSQL.
</Callout>
