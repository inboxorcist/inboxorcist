    ...
    <div className="flex items-center gap-2 px-3 py-2 bg-card border rounded-lg">
      ...
      <div className="flex flex-col">
        <span className="text-xs font-medium text-foreground/80">
          {isRefreshing ? "Syncing..." : refreshError ? "Sync failed" : formatRelativeTime(lastSyncTime)}
        </span>
      ...
      >
        <RefreshCw className={`h-3.5 w-3.5 text-muted-foreground group-hover:text-foreground transition-colors ${isRefreshing ? "animate-spin" : ""}`} />
      </button>
    </div>
  );
}
